...
import com.example.recv_deeplink.BuildConfig;
...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m1600constructorimpl = SegmentOrClosed.m1600constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1600constructorimpl = SegmentOrClosed.m1600constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1605isClosedimpl(m1600constructorimpl)) {
                break;
            }
            Segment m1603getSegmentimpl = SegmentOrClosed.m1603getSegmentimpl(m1600constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= m1603getSegmentimpl.getId()) {
                    break;
                } else if (!m1603getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    continue;
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1603getSegmentimpl)) {
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1603getSegmentimpl(m1600constructorimpl);
