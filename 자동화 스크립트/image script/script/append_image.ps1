...
Param (
    ...
    if ( $border -eq "b" ) {
        magick convert $img_file -bordercolor black -border 2 ("border_"+$img_file)
        magick convert $cache_file_name -bordercolor black -border 2 $cache_file_name
    }
    ...
    if ( $border -eq "b") {
        ### 가로
        if ( $direction -eq "left" -and $img_dir -eq "w" ) { magick convert ("border_"+$img_file) $cache_file_name +append $result_img_name }
        elseif ( $direction -eq "right" -and $img_dir -eq "w" ) { magick convert $cache_file_name ("border_"+$img_file) +append $result_img_name }
        ### 세로
        if ( $direction -eq "left" -and $img_dir -eq "h" ) { magick convert $cache_file_name ("border_"+$img_file) -append $result_img_name }
        elseif ( $direction -eq "right" -and $img_dir -eq "h" ) { magick convert ("border_"+$img_file) $cache_file_name -append $result_img_name }
        remove-item ("border_"+$img_file)

    } else {
        ### 가로
        if ( $direction -eq "left" -and $img_dir -eq "w" ) { magick convert $img_file $cache_file_name +append $result_img_name }
        elseif ( $direction -eq "right" -and $img_dir -eq "w" ) { magick convert $cache_file_name $img_file +append $result_img_name }
        ### 세로
        if ( $direction -eq "left" -and $img_dir -eq "h" ) { magick convert $cache_file_name $img_file -append $result_img_name }
        elseif ( $direction -eq "right" -and $img_dir -eq "h" ) { magick convert $img_file $cache_file_name -append $result_img_name }
    }
    
    # 임시 이미지 파일 삭제
    Remove-Item $cache_file_name

    # 여백 없이 붙이기일 경우 (white_space=0) → 중앙 라인 1px 추가
    if ( [int]$white_space -eq 0 ) {
        $result_img_x = magick identify -format '%w' $result_img_name
        $result_img_y = magick identify -format '%h' $result_img_name

        if ( $img_dir -eq "w" ) {
            $half_x = [int]$result_img_x / 2
            magick convert $result_img_name -stroke black -strokewidth 1 -draw "line $half_x,0 $half_x,$result_img_y" $result_img_name
        } elseif ( $img_dir -eq "h" ) {
            $half_y = [int]$result_img_y / 2
            magick convert $result_img_name -stroke black -strokewidth 1 -draw "line 0,$half_y $result_img_x,$half_y" $result_img_name
        }
    }
...
if ( $file_name1 -ne "" -and $file_name2 -ne "" ) {
    ...
    echo ""
    echo "파일 이름을 입력하세요." 
    echo 'example> append_image.ps1 [img1.png], [img2.png], [direction(w,h)], [white_space(default-10)] [orignal img delete(d)] [border(b)]'
    echo "example> append_image.ps1 a.png b.png w 0"
    echo ""
}
