...
%>				
			<tr>	
				...
				<td><%=bean.getRday() %></td>
				<td><%=bean.getDday() %></td>
				<td><%= ( bean.getPrice()* bean.getQty() * bean.getDday() ) + ( bean.getQty()*bean.getDday())*( (bean.getUserin() * 10000) + ( bean.getUsewifi() * 10000 ) + ( bean.getUseseat() * 10000 ) ) %> 원</td>
				<td><%=bean.getQty() %></td>
				<td><%=(bean.getUserin()  == 1) ? "적용" : "비적용" %></td>
				<td><%=(bean.getUsewifi() == 1) ? "적용" : "비적용" %></td>
				<td><%=(bean.getUsenavi() == 1) ? "적용" : "비적용" %></td>
				<td><%=(bean.getUseseat() == 1) ? "적용" : "비적용" %></td>
